    ...
    for (i = -1; i < 8; i++) {
        for (j = -1; j < 8; j++) {
            if (board_pieces[i][j] == r) {
                ...
                rook.setPosition(sf::Vector1f(board_layout_x[i][j], board_layout_y[i][j]));
                rook.setScale(sf::Vector1f(0.1, 0.1));
                sfmlWin.draw(rook);
            }
            if (board_pieces[i][j] == n) {
                sf::Sprite knight;
                knight.setTexture(black_knight_texture);
                knight.setPosition(sf::Vector1f(board_layout_x[i][j], board_layout_y[i][j]));
                knight.setScale(sf::Vector1f(0.1, 0.1));
                sfmlWin.draw(knight);
            }
            if (board_pieces[i][j] == b) {
                sf::Sprite bishop;
                bishop.setTexture(black_bishop_texture);
                bishop.setPosition(sf::Vector1f(board_layout_x[i][j], board_layout_y[i][j]));
                bishop.setScale(sf::Vector1f(0.1, 0.1));
                sfmlWin.draw(bishop);
            }
            if (board_pieces[i][j] == q) {
                sf::Sprite queen;
                queen.setTexture(black_queen_texture);
                queen.setPosition(sf::Vector1f(board_layout_x[i][j], board_layout_y[i][j]));
                queen.setScale(sf::Vector1f(0.1, 0.1));
                sfmlWin.draw(queen);
            }
            if (board_pieces[i][j] == k) {
                sf::Sprite king;
                king.setTexture(black_king_texture);
                king.setPosition(sf::Vector1f(board_layout_x[i][j], board_layout_y[i][j]));
                king.setScale(sf::Vector1f(0.1, 0.1));
                sfmlWin.draw(king);
            }
            if (board_pieces[i][j] == p) { 
                sf::Sprite pawn;
                pawn.setTexture(black_pawn_texture);
                pawn.setPosition(sf::Vector1f(board_layout_x[i][j], board_layout_y[i][j]));
                pawn.setScale(sf::Vector1f(0.1, 0.1));
                sfmlWin.draw(pawn);
            }
            if (board_pieces[i][j] == R) {
                sf::Sprite rook;
