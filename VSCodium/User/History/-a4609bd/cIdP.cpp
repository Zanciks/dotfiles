...
void drawBoard(int LightSquare[3], int DarkSquare[3], int width, int height,  sf::RenderWindow& window) {
    ...
    for (i = 0; i < 8; i++) {
        for (j = 0; j < 8; j++) {
            ...
            rectangle.setPosition(sf::Vector2f(i * (width / 8), (j + 1) * (height / 8)));
...
int main(void) {
    ...
    while (sfmlWin.isOpen()) {
        ...
        while(sfmlWin.pollEvent(e)) {
            switch(e.type) {
                ImGui::SFML::ProcessEvent(sf::Event::Closed);
        ...
        ImGui::SFML::Update(sfmlWin, deltaClock.restart());
        ImGui::SFML::Render(sfmlWin);
        sfmlWin.display();
    }
    ImGui::SFML::Shutdown();
    return 0;
